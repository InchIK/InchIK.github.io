(async () => {
  ...
  const VISUALS = {
    ...
    network: {
      ...
      image: "assets/photos/network.jpg",
    },
    firewall: {
      label: "Security & Firewalls",
      image: "assets/photos/firewall.jpg",
    },
    switching: {
      label: "Switching Infrastructure",
      image: "assets/photos/switching.jpg",
    },
    os: { label: "Operating Systems", image: "assets/photos/os.jpg" },
    virtualization: {
      label: "Virtualization & Cloud",
      image: "assets/photos/virtualization.jpg",
    },
    database: {
    ...
    monitoring: {
      ...
      image: "assets/photos/monitoring.jpg",
    },
    coding: {
      label: "Scripting & Automation",
      image: "assets/photos/coding.jpg",
    },
    default: {
      label: "Skill Illustration",
  ...
  const projectNavigationList = document.getElementById(
    "projectNavigationList"
  );
  const detailProjectContainer = document.getElementById(
  ...
  const experienceManagerList = document.getElementById(
  ...
  const certificateManagerList = document.getElementById(
  ...
  const shareLinksManagerList = document.getElementById(
    "shareLinksManagerList"
  );
  ...
  async function loadRemoteData() {
    ...
    console.warn(
      "Unable to load resume data from Firestore or static JSON; falling back to defaults."
    );
  ...
  function normalizeData(source) {
    ...
    return {
      profile: {
        ...DEFAULT_PROFILE,
        ...(typeof source.profile === "object" && source.profile
          ? source.profile
          : {}),
      },
  ...
  function normalizeAboutMe(source) {
    ...
    return {
      ...
      positions: Array.isArray(source.positions)
        ? source.positions.filter((p) => typeof p === "string" && p.trim())
        : [],
      education: Array.isArray(source.education)
        ? source.education.map((edu, i) => ({
            id: edu.id || `edu-${i + 1}`,
            school: edu.school || "",
            degree: edu.degree || "",
          }))
      ...
      certificates: Array.isArray(source.certificates)
        ? source.certificates.map((cert, i) => ({
            id: cert.id || `cert-${i + 1}`,
            title: cert.title || "",
  ...
  function normalizeAdminTags(source) {
    ...
    return source
      .map((tag) => {
        ...
        const frontBg =
          typeof tag.frontBg === "string" ? tag.frontBg.trim() : "";
  ...
  function normalizeSkills(skillsSource) {
    ...
    return list.map((skill, index) => {
      ...
      const visual =
        typeof sanitized.visual === "string" ? sanitized.visual.trim() : "";
      const derivedImage =
        visual && VISUALS[visual]?.image ? VISUALS[visual].image : "";
  ...
  function serializeForFirestore(payload) {
    ...
    const sanitizedSkills = (payload.skills ?? []).map((skill, index) => {
      const visual =
        typeof skill.visual === "string" ? skill.visual.trim() : "";
      const base = {
        id:
    ...
    const sanitizedAboutMe = {
      ...
      experiences: Array.isArray(payload.aboutMe?.experiences)
        ? payload.aboutMe.experiences
        : [],
      positions: Array.isArray(payload.aboutMe?.positions)
        ? payload.aboutMe.positions
        : [],
      education: Array.isArray(payload.aboutMe?.education)
        ? payload.aboutMe.education
        : [],
      certificates: Array.isArray(payload.aboutMe?.certificates)
        ? payload.aboutMe.certificates
        : [],
      websites: Array.isArray(payload.aboutMe?.websites)
        ? payload.aboutMe.websites
        : [],
    };
  ...
  function getRandomAdminTagColors(frontCandidate, backCandidate) {
    const frontCandidateTrimmed =
      typeof frontCandidate === "string" ? frontCandidate.trim() : "";
    const backCandidateTrimmed =
    ...
    const frontBg =
      frontCandidateTrimmed && frontCandidateTrimmed.toLowerCase() !== "#000000"
        ...
        : ADMIN_TAG_FRONT_COLORS[
            Math.floor(Math.random() * ADMIN_TAG_FRONT_COLORS.length)
          ];
    const backBg =
      backCandidateTrimmed && backCandidateTrimmed.toLowerCase() !== "#000000"
        ? backCandidateTrimmed
        : ADMIN_TAG_BACK_COLORS[
            Math.floor(Math.random() * ADMIN_TAG_BACK_COLORS.length)
          ];
  ...
  function createAdminTagElement(tag) {
    ...
    const { frontBg, backBg } = getRandomAdminTagColors(
      tag.frontBg,
      tag.backBg
    );
  ...
  function renderExperiences() {
    ...
    if (!aboutMe.experiences || aboutMe.experiences.length === 0) {
      experienceListEl.innerHTML =
        '<div style="margin-left: 1rem; color: rgba(28, 36, 49, 0.5);">暫無經歷</div>';
      return;
    }
  ...
  function renderPositions() {
    ...
    if (!aboutMe.positions || aboutMe.positions.length === 0) {
      positionListEl.innerHTML =
        '<div style="color: rgba(28, 36, 49, 0.5);">暫無職務</div>';
      return;
    }
  ...
  function renderEducation() {
    ...
    if (!aboutMe.education || aboutMe.education.length === 0) {
      educationListEl.innerHTML =
        '<div style="color: rgba(28, 36, 49, 0.5);">暫無學歷</div>';
      return;
    }
  ...
  function renderCertificates() {
    ...
    if (!aboutMe.certificates || aboutMe.certificates.length === 0) {
      certificateListEl.innerHTML =
        '<div style="color: rgba(28, 36, 49, 0.5);">暫無證書</div>';
      return;
    }
  ...
  function renderWebsites() {
    ...
    if (!aboutMe.websites || aboutMe.websites.length === 0) {
      websiteListEl.innerHTML =
        '<div style="color: rgba(28, 36, 49, 0.5);">暫無網站連結</div>';
      return;
    }
  ...
  function renderShareLinks() {
    ...
    const cardWidth =
      (shareLinksGrid.offsetWidth - gap * (columnsPerRow - 1)) / columnsPerRow;
    ...
    if (enableWaterfall && totalCount > regularCardsCount) {
      ...
      for (let col = 0; col < columnsPerRow; col++) {
        const missing = Math.floor(Math.random() * 5) + 1; // 1-5
      ...
      if (totalAssigned > remainingCards) {
        ...
        for (let i = 0; i < excess; i++) {
          ...
          extraCardsPerColumn[maxCol] = Math.max(
            0,
            extraCardsPerColumn[maxCol] - 1
          );
        }
      }
  ...
  editProfileBtn?.addEventListener("click", () => {
    ...
    profileAvatarInput.value =
      profile.avatar === DEFAULT_PROFILE.avatar ? "" : profile.avatar;
  ...
  profileForm?.addEventListener("submit", (event) => {
    ...
    profile = {
      ...
      avatar: rawAvatarUrl
        ? convertGoogleDriveUrl(rawAvatarUrl)
  ...
  projectListEl?.addEventListener("click", (event) => {
    ...
    if (action === "edit") {
      const projectToEdit = skill.projects.find(
  ...
  projectForm?.addEventListener("submit", (event) => {
    ...
    if (editingProjectId) {
      const projectIndex = skill.projects.findIndex(
  ...
  // 回到頂部按鈕功能
  const skillDetailContent = document.querySelector(
  ...
  function renderProjectManager() {
    ...
    const sorted = [...skill.projects].sort((a, b) =>
                    ...
                    <article class="project-item" data-project-id="${
                      project.id
                        ...
                        <div class="project-item__header">
                            <div class="project-item__name">${escapeHTML(
                              project.name
                            )}</div>
                            <div class="project-item__actions">
                                <button type="button" class="admin-btn" data-project-action="edit" data-project-id="${
                                  project.id
                                }">Edit</button>
                                <button type="button" class="admin-btn admin-btn--quiet" data-project-action="delete" data-project-id="${
                                  project.id
                                }">Delete</button>
                            </div>
                        </div>
                        <div class="project-item__intro">${escapeHTML(
                          plain
                        )}</div>
                        <div class="project-item__meta">Media: ${
  ...
  function openProjectEditor(project) {
    ...
    projectEditorTitle.textContent = editingProjectId
      ? "Edit Project"
      : "Add Project";
  ...
  function renderSkillDetail(skill) {
    ...
    detailSkillIntro.innerHTML = `<p>${escapeHTML(
      skill.description || ""
    )}</p>`;
    ...
    // 為導航按鈕添加點擊事件
    projectNavigationList
      .querySelectorAll(".project-nav-item")
      .forEach((navItem) => {
        navItem.addEventListener("click", () => {
          const projectId = navItem.dataset.projectId;
          const targetProject = detailProjectContainer.querySelector(
          ...
          if (targetProject) {
            ...
            targetProject.style.boxShadow =
              "0 0 0 3px rgba(12, 124, 213, 0.3), 0 8px 24px rgba(0, 0, 0, 0.1)";
  ...
  function renderProjectMedia(media) {
    const label = media.label
      ? `<div class="project-media-label">${escapeHTML(media.label)}</div>`
      : "";
      ...
      return `
                <div class="project-media-card project-media-card--file">
                    ...
                    <a href="${escapeAttribute(
                      fileUrl
                    )}" target="_blank" rel="noopener noreferrer" data-media-type="file"><img src="assets/photos/file.svg" alt="file icon" class="file-icon">${escapeHTML(
  ...
  function addExperienceItem(data = null) {
    ...
    div.innerHTML = `
            <div class="manager-item__fields">
                <input type="text" placeholder="公司名稱" value="${escapeHTML(
                  data?.company || ""
                )}" data-field="company" />
                <input type="text" placeholder="年資 (例: 3年)" value="${escapeHTML(
                  data?.years || ""
                )}" data-field="years" />
                <input type="text" placeholder="技能 (逗號分隔)" value="${
    ...
    div
      .querySelector(".manager-item__remove")
  ...
  function renderPositionManager() {
    ...
    input.placeholder =
      "職務名稱（使用逗號分隔，例如：DevOps, 雲端架構師, 伺服器管理員）";
  ...
  function addEducationItem(data = null) {
    ...
    div.innerHTML = `
            <div class="manager-item__fields">
                <input type="text" placeholder="學校名稱" value="${escapeHTML(
                  data?.school || ""
                )}" data-field="school" />
                <input type="text" placeholder="學位/科系" value="${escapeHTML(
                  data?.degree || ""
                )}" data-field="degree" />
            </div>
            <button type="button" class="manager-item__remove">×</button>
  ...
  function addCertificateItem(data = null) {
    ...
    div.innerHTML = `
            <div class="manager-item__fields">
                <input type="text" placeholder="證書名稱" value="${escapeHTML(
                  data?.title || ""
                )}" data-field="title" />
                <input type="text" placeholder="圖片網址" value="${escapeHTML(
                  data?.imageUrl || ""
                )}" data-field="imageUrl" />
                <input type="text" placeholder="說明" value="${escapeHTML(
                  data?.description || ""
  ...
  function collectCertificates() {
    ...
    return Array.from(items)
      .map((item) => {
        const rawImageUrl =
          item.querySelector('[data-field="imageUrl"]')?.value.trim() || "";
        return {
  ...
  function addWebsiteItem(data = null) {
    ...
    div.innerHTML = `
            <div class="manager-item__fields">
                <input type="text" placeholder="網站名稱 (例: GitHub)" value="${escapeHTML(
                  data?.label || ""
                )}" data-field="label" />
                <input type="text" placeholder="網址" value="${escapeHTML(
                  data?.url || ""
                )}" data-field="url" />
                <input type="text" placeholder="圖示網址 (選填)" value="${escapeHTML(
                  data?.icon || ""
                )}" data-field="icon" />
            </div>
            <button type="button" class="manager-item__remove">×</button>
  ...
  function addShareLinkItem(data = null) {
    ...
    div.innerHTML = `
            <div class="share-link-manager-item__fields">
                <input type="text" placeholder="分享名稱（例如：GitHub 或 個人部落格）" value="${escapeHTML(
                  data?.name || ""
                )}" data-field="name" />
                <input type="text" placeholder="說明（hover時顯示）" value="${escapeHTML(
                  data?.description || ""
                )}" data-field="description" />
                <input type="text" placeholder="連結網址" value="${escapeHTML(
                  data?.url || ""
                )}" data-field="url" />
            </div>
    ...
    div
      .querySelector(".share-link-manager-item__remove")
      ?.addEventListener("click", () => div.remove());
  ...
  function collectShareLinks() {
    if (!shareLinksManagerList) return [];
    const items = shareLinksManagerList.querySelectorAll(
  ...
  function addTellMeItem(value = "") {
    ...
    div.innerHTML = `
            <input type="text" placeholder="輸入項目內容" value="${escapeHTML(
              value
            )}" data-field="item" />
    ...
    div
      .querySelector(".tell-me-item__remove")
      ?.addEventListener("click", () => div.remove());
  ...
  function collectTellMe() {
    ...
    const list = Array.from(items)
      .map(
        (item) => item.querySelector('[data-field="item"]')?.value.trim() || ""
      )
